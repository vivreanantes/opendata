...
Ext.define('VivreANantes.controller.GarbagesController', {
	...
	putInButtonsPanel : function (stringFilter) {
		...
		var arrayItemsToShow = this.getArrayItemsToShowForButtons(commonDatasUsualCategory, stringFilter);
		var result = new Array();
		if (arrayItemsToShow.length > 0) {

			var theItems = arrayItemsToShow;
			for (var i = 0; i < theItems.length; i++) {
				var stLibelle = this.decoupeAvecTaille(theItems[i]["libelle"], 20);
				result.push({
					code : theItems[i].id,
					label : stLibelle,
	...
	translateWithUpperFirstLetterAndDecoupe : function (stLabel) {
		return this.decoupeAvecTaille(this.translateWithUpperFirstLetter(stLabel), 20);
	},
		...
		if (arrayConseils.length > 0) {
			...
			for (j in commonDatasAdvices) {
				if (commonDatasAdvices[j]["code"] === arrayConseils[i]) {
					conseilTraduit = conseilTraduit + "<BR/>" + thisController.translate("label_Conseil") + " : <B>" + commonDatasAdvices[j]["libelle"] + "</B><BR/>" + commonDatasAdvices[j]["description"];
					if (commonDatasAdvices[j]["fiche"] !== "") {
						conseilTraduit += thisController.makeTextLink_old("informationsPanel");
					}
				}
			}
...
//			});
	...
	showGarbagesDetail : function (button, e, eOpts) {
		...
		if (record) {
			...
			// var title = "<I>" + this.translate("label_dechet") + "</I> " + record["nom"];
			...
			// Modes de collecte
			this.garbageDetail.items.items['0'].items.items['1'].items.items['0'].setData({
				...
				for (j in commonDatasCollectMods) {
					for (i in arrayModesDeCollecte) {
						if (commonDatasCollectMods[j]["code"] === arrayModesDeCollecte[i]) {
							...
							var codeValue = "collectMods_xtype" + thisController.SEPARATOR + commonDatasCollectMods[j]["code"];
							var libelleValue = this.stringUpperFirstLetter(commonDatasCollectMods[j]["libelle"]);
							arrayItemsToShow.push({
								image : imageValue,
								code : codeValue,
								label : libelleValue
			...
			this.setDatasConseils(this.garbageDetail.items,	"garbagesdetails_conseils", "libelle", arraysConseils.les_libelles, nbElementsMax);
			...
			// title));
			var arraysCommentaires = this.getArrayItemsToShowComments(code,	record["nom"]);
			var nbElementsMax = 3;
			// la page GarbagesDetails.js  affiche 3 éléments
			this.setDataInButtons(this.garbageDetail, "garbagesdetails_commentaires", arraysCommentaires.les_libelles, nbElementsMax);
			var index = this.garbageDetail.items.keys.indexOf("garbagesdetails_envoyer");
			this.garbageDetail.items.items[index].setData({	code : arraysCommentaires.le_titre });
			/*
...
//		// STORE datasGarbages
//		var datas = this.getApplication().getController("VivreANantes.controller.GarbagesController").getGarbagesList().getStore().getData();
...
//		});
	...
	filter2 : function () {
		...
		if (this.garbagesList == null) {
			this.garbagesList = Ext.create('VivreANantes.view.garbages.GarbagesList');
		}
		...
		for (j in commonDatasGarbages) {
			...
			if ((commonDatasGarbages[j]["categoriesUsuelles"] === category.getValue() || category
					.getValue() === "all")
				 && texttest.test(nom_description_sansAccents)) {
				// Ajoute les <br/>
				var stLibelle = this.decoupeAvecTaille(commonDatasGarbages[j]["nom"], 20);
				result.push({
		...
		if (this.garbagesButtonsPanel == null) {
			this.garbagesButtonsPanel = Ext.create('VivreANantes.view.garbages.GarbageButtonsPanel');
		}
		...
		this.setDataInButtonsWithManyLines(this.garbagesButtonsPanel, "garbagesButtonsPanel_garbage", result, nbGarbagesMax, 3);
