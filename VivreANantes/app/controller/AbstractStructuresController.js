...
Ext.define("VivreANantes.controller.AbstractStructuresController", {
	...
	getAttributes_FillPeriod : function(objStructures, stLocale) {
		...
		if (stPlagesHoraire != null && stPlagesHoraire != "") {
			...
			var saufFerie = this.utilArrayContainObject(arPlagesHoraires,
					this.SAUF_FERIE);
			...
			for (var i = 0; i < arPlagesHoraires.length; i++) {
				...
				if (plageHoraire == this.SAUF_FERIE) {
					...
					// -- verifie si
					var obAujoudhuiDemain = this.verifieOuvertAujourdhuiDemain(dateDebut, dateFin, daysOfWeekZone, saufFerie);
	...
	, verifieOuvertAujourdhuiDemain : function(dateDebut, dateFin, daysOfWeekZone, saufFerie) {

		var today = this.utilGetDateWithoutSeconds();
		...
		if (today >= dateDebut && today <= dateFin && this.utilArrayContainObject(arDays, todayTwoDays)) {
			bOuvertAujourdhui = true;
		}
		
		if (tomorrow >= dateDebut && tomorrow <= dateFin && this.utilArrayContainObject(arDays, tomorrowTwoDays)) {
			bOuvertDemain = true;
		}

		if (saufFerie) {
			if (this.verifFerie(saufFerie, today)) {
				bOuvertAujourdhui = false;
			}
			if (this.verifFerie(saufFerie, tomorrow)) {
				bOuvertDemain = false;
			}
		}
		
		return { "bOuvertAujourdhui" : bOuvertAujourdhui, "bOuvertDemain" : bOuvertDemain } 	
	}
