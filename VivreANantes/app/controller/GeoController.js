...
Ext.define('VivreANantes.controller.GeoController', {
	...
	onMapRender : function(extmap, map, eOpts) {
		...
		/*
		 * if (!Ext.isDefined(me.geolocate)) { me.geolocate =
		 * Ext.create('Ext.util.Geolocation', { autoUpdate : true, listeners : {
		 * locationupdate : function(geo) { me.resetCenter(geo); },
		 * locationerror : function(geo, bTimeout, bPermissionDenied,
		 * bLocationUnavailable, message) { if (bTimeout) {
		 * console.error('Timeout'); } else { console.error('Erreur :('); } } }
		 * }); }
		 */
		...
		if (!Ext.isDefined(this.structureStore)) {
			this.structureStore = Ext.create(
					'VivreANantes.store.StructureStoreGeo', {
						autoLoad : true,
						listeners : {
							'load' : function(store, results, successful) {
								console.log("Loading StructureStoreGeo");
