...
Ext.define('VivreANantes.controller.WelcomeController', {
	...
	onInitWelcome2 : function(element, eOpts) {
		...
		/*
		 * var is_chrome = navigator.userAgent.toLowerCase().indexOf('chrom') >
		 * -1; if (is_chrome) { element.items.items[0].setHtml(titre+text); }
		 * else { element.items.items[0].setHtml(titre+utiliserChrome+text); }
		 */
	}
