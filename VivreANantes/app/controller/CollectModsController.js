Ext.define('VivreANantes.controller.CollectModsController', {
			...
			onActivate : function(newActiveItem, container, oldActiveItem,
				...
				// var dataCollectMods =
				// this.getApplication().getController("VivreANantes.controller.GarbagesController").getCollectModList().getStore();
				// var arrayItemsToShow =
				// this.getDatasForButtons_old(dataCollectMods, "modco");

				var arrayItemsToShow = this.getArrayItemsToShowForButtons(
						commonDatasCollectMods, "modco");
				...
				this.setDataInButtonsWithManyLines(this.getCollectModsList(),
						"collectModsButtonsList", result, nbMax, 3);

				// var arrayItems =
				// this.getContentButtonsPanel(arrayItemsToShow);
				// this.removeAllAndSetItems(this.getCollectModsList(),
				// arrayItems);
			},
			...
			showDetails : function(elementId) {
				...
				// "</I> " +
				// this.stringUpperFirstLetter(collectModFromStore["libelle"]);
				// this.collectModsDetails.setTitle(title);
				this.collectModsDetails
						.setTitle(this
				...
				this.setItemsElement(this.collectModsDetails,
						"collectModsDetails_comments", this.getItemsComments(
								code, collectModFromStore["libelle"]));
			...
			getElementFromStore : function(idElement) {
				...
				// // STORE dataCollectMods
				// var datas =
				// this.getApplication().getController("VivreANantes.controller.GarbagesController").getCollectModList().getStore().getData();
